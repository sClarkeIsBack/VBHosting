...
def start():
	if username=="":
		...
		if auth == "":
			...
			addonsettings('ADS2','')
			xbmc.executebuiltin('Container.Refresh')
			xbmcgui.Dialog().ok(user.name, 'Setup Complete', 'Enjoy Your Stay With Us At %s'%user.name, 'If You Have Any Problems, Contact Us At: %s'%user.name)
			home()
	else:
...
def correctPVR():
	...
	EPGurl     = 'https://megaiptv.co/epg/guide.xml'
